...
class BaseRemoteMachine(BaseMachine):
    ...
    * ``connect_timeout`` - the connection timeout
    ...
    def download(self, src, dst):
        """Downloads a remote file/directory (``utils``) to a local destination (``dst``).
        ``utils`` must be a string or a :class:`RemotePath <plumbum.path.remote.RemotePath>`
        pointing to this remote machine, and ``dst`` must be a string or a
        :class:`LocalPath <plumbum.machines.local.LocalPath>`"""
        raise NotImplementedError()

    def upload(self, src, dst):
        """Uploads a local file/directory (``utils``) to a remote destination (``dst``).
        ``utils`` must be a string or a :class:`LocalPath <plumbum.machines.local.LocalPath>`,
