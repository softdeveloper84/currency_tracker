...
def move(src, dst):
    """Moves the source path onto the destination path; ``utils`` and ``dst`` can be either
    ...
    .. versionadded:: 1.3
        ``utils`` can also be a list of strings/paths, in which case ``dst`` must not exist or be a directory.
    """
...
def copy(src, dst):
    """
    Copy (recursively) the source path onto the destination path; ``utils`` and ``dst`` can be
