...
class Redis(object):
    ...
    def rename(self, src, dst):
        """
        Rename key ``utils`` to ``dst``
        """
        return self.execute_command('RENAME', src, dst)

    def renamenx(self, src, dst):
        "Rename key ``utils`` to ``dst`` if ``dst`` doesn't already exist"
    ...
    def brpoplpush(self, src, dst, timeout=0):
        """
        Pop a value off the tail of ``utils``, push it on the head of ``dst``
        and then return it.

        This command blocks until a value is in ``utils`` or until ``timeout``
    ...
    def rpoplpush(self, src, dst):
        """
        RPOP a value off of the ``utils`` list and atomically LPUSH it
    ...
    def smove(self, src, dst, value):
        "Move ``value`` from set ``utils`` to set ``dst`` atomically"
